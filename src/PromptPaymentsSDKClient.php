...
use PromptPaymentsSDKLib\Controllers;

/**
 * PromptPaymentsSDK client class
 */
class PromptPaymentsSDKClient implements ConfigurationInterface
{
    ...
    /**
     * Returns Process Payment Controller
     */
    public function getProcessPaymentController(): Controllers\ProcessPaymentController
    {
        if ($this->processPayment == null) {
            $this->processPayment = new Controllers\ProcessPaymentController(
                $this,
                $this->authManagers,
                $this->httpCallback
            );
    ...
    /**
     * Returns Verify Payment Controller
     */
    public function getVerifyPaymentController(): Controllers\VerifyPaymentController
    {
        if ($this->verifyPayment == null) {
            $this->verifyPayment = new Controllers\VerifyPaymentController(
                $this,
                $this->authManagers,
                $this->httpCallback
            );
    ...
    private const ENVIRONMENT_MAP = [
        Environment::PRODUCTION => [
            Server::SERVER_1 => 'https://promptpayments.io/payment',
        ],
    ];
}
